...
class BatchProcessor(IBatchProcessor[I, O]):
    def __init__(
        ...
        #Crear un evento para cuando esto este activado, mandarlo desde wl worker y parar todos los workers
    ...
    def _worker_target(self) -> None:
        ...
        while not self.stop_event.is_set() or not self.in_queue.empty():
            #Un if que si el evento de stop_on_worker_death esta activo break
            try:
    ...
    # ------------------ Monitor ------------------
    def _monitor_loop(self, interval: float = 0.0005) -> None:
