...
class WorkerReportedError(WorkerError):
    def __init__(self, info: ExceptionInfo):
        super().__init__(f"Worker reported exception {info.exc_type}: {info.message}\n{info.tb}")
        self.info = info
...
class BatchProcessor(IBatchProcessor[I, O]):
        ...
        while not self.stop_event.is_set():
            ...
            try:
                ...
                try:
                    ...
                    logger.exception("Failed to put ExceptionInfo into error_queue; re-raising")
                    raise
                logger.warning("Worker reported exception and continues: %s", info.exc_type)
    ...
    def start(self) -> None:
        ...
        if self.restart_dead_workers:
            self.monitor_thread = Thread(target=self._monitor_workers, daemon=True)
    ...
    def stop(self) -> None:
        ...
        self._join_workers()
        self._handle_reported_exceptions()
        self._handle_fatal_workers()
        self._cleanup_workers()

    # ------------------ Monitor ------------------
    def _monitor_workers(self, interval: float = 0.5) -> None:
        while not self.stop_event.is_set():
            with self.worker_lock:
    ...
    def _handle_reported_exceptions(self) -> None:
        infos = self.poll_exceptions()
        if infos:
            first = infos[0]
            logger.error("Worker reported exception: %s: %s", first.exc_type, first.message)
            if self.stop_on_reported_exception:
                with self.worker_lock:
                    ...
                    self.workers.clear()
                raise WorkerReportedError(first)

    def _handle_fatal_workers(self) -> None:
        with self.worker_lock:
            for p in self.workers:
                if p.exitcode is not None and p.exitcode != 0:
                    fatal = WorkerFatalError(p.pid, p.exitcode)
                    logger.error("Worker died unexpectedly: pid=%s exitcode=%s", fatal.pid, fatal.exitcode)
